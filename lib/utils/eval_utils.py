...
def compute_error_verts_torch(pred_verts, target_verts=None):
    """
    Computes MPJPE over 6890 surface vertices.
    Args:
        verts_gt (Nx6890x3).
    ...
    assert len(pred_verts) == len(target_verts)
    error_per_vert = torch.sqrt(torch.sum((target_verts - pred_verts) ** 2, axis=2))
    return torch.mean(error_per_vert, axis=1)
...
def batch_compute_similarity_transform_torch(S1, S2):
    ...
    '''
    transposed = False
    # if S1.shape[0] != 3 and S1.shape[0] != 2: # error issued from https://github.com/Arthur151/ROMP/blob/master/simple_romp/trace2/PMPJPE_BUG_REPORT.md
