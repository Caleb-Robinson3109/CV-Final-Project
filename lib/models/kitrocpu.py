...
def batch_cal_swing(t, p): 
    ...
    zeros = torch.zeros((batch_size, 1, 1)).to(device)

    K = torch.cat([zeros, -rz, ry, rz, zeros, -rx, -ry, rx, zeros], dim=1).view((batch_size, 3, 3))
    ident = torch.eye(3).unsqueeze(dim=0).to(device)
...
def batch_cal_2solutions(Ap, Bp, OA_len, bone_len, inv_intrinsics):
    ...
    scale = torch.sqrt(torch.where(inner > 0, inner, torch.tensor(0).float().to(device))) 
...
def batch_cal_2d_error_after_align(refine_output_i_joints, chain_index, cam_R, cam_t, intrinsics, keypoints_2d, alignjid):
    ...
    myj2d = torch.cat((myj2d, torch.ones(myj2d.shape[0], myj2d.shape[1], 1).type_as(myj2d)), dim=-1).squeeze(0).float().to(device)[:,J24_INDEX_IN_59]
...
def optimize_shape(pred_shape, refined_thetas, refined_cam, intrinsics, keypoints_2d, smpl, num_iterations = 50):
    ...
    myj2d = perspective_projection(refine_output_curr.joints, torch.eye(3).repeat(batch_size,1,1).to(device), refined_cam, intrinsics)
    myj2d = torch.cat((myj2d, torch.ones(myj2d.shape[0], myj2d.shape[1], 1).type_as(myj2d)), dim=-1).float().to(device)
    ...
    with torch.enable_grad():
        ...
        rotation_matrix = torch.eye(3).repeat(batch_size, 1, 1).to(device)
...
def solution_calculation_chain(refine_output_curr, inv_intrinsics, keypoints_2d, reproj2d, reference_joints, Oparent_len, chain):
    ...
    result = {}
    similarity = torch.ones(batch_size)[:,None].to(device)
    rotmat = torch.eye(3)[None, None, :].repeat(batch_size,1,1,1).to(device)
    for child in chain:
...
def solution_selection(refined_thetas, refined_shape, smpl, refined_cam, intrinsics, inv_intrinsics, keypoints_2d, reference_joints):
    ...
    reproj2d = perspective_projection(refine_output_curr.joints, torch.eye(3).repeat(batch_size,1,1).to(device), refined_cam, intrinsics)
    reproj2d = torch.cat((reproj2d, torch.ones(reproj2d.shape[0], reproj2d.shape[1], 1).type_as(reproj2d)), dim=-1).float().to(device)[:,J24_INDEX_IN_59]

    selection = -torch.ones([batch_size,24]).long().to(device)
...
def refine_alignKT(refined_thetas, refined_shape, smpl, refined_cam, intrinsics, inv_intrinsics, keypoints_2d, selection, origin_thetas):
    ...
    for i in range(24):
        ...
        # Align 2D based on parent joint
        myj2d = perspective_projection(refine_output_curr.joints, torch.eye(3).repeat(batch_size,1,1).to(device), refined_cam, intrinsics)
        myj2d = torch.cat((myj2d, torch.ones(myj2d.shape[0], myj2d.shape[1], 1).type_as(myj2d)), dim=-1).float().to(device)[:,J24_INDEX_IN_59]
        ...
        if i == 0 or i == 9: # joint 0 and 9 has 3 child joints.
            ...
            if i==0:
                R_parent = torch.eye(3).repeat(batch_size,1,1).to(device)
            else:
                j = PARENTS[i]
        ...
        old_2d_err = batch_cal_2d_error_after_align(refine_output_curr.joints, torch.tensor([ii for ii in already_refined_index]).to(device), torch.eye(3).repeat(batch_size,1,1).to(device), refined_cam, intrinsics, keypoints_2d, alignjid=i)
        refine_output_curr = smpl(
            betas=refined_shape,
            body_pose=refined_thetas[:, 1:],
            global_orient=refined_thetas[:, 0].unsqueeze(1),
        ...
        new_2d_err = batch_cal_2d_error_after_align(refine_output_curr.joints, torch.tensor([ii for ii in already_refined_index]).to(device), torch.eye(3).repeat(batch_size,1,1).to(device), refined_cam, intrinsics, keypoints_2d, alignjid=i)
...
def KITRO_refine(batch_size, init_smpl_estimate, evidence_2d, J_regressor=None, kitro_cfg=None, smpl=None):
    ...
    ############## 2D Evidence ###############
    print(f"{np.shape(evidence_2d['keypoints_2d'])} {evidence_2d.keys()}")
    keypoints_2d = evidence_2d['keypoints_2d']#[:,J24_INDEX_IN_59] # [b,24,2]
    keypoints_2d = torch.cat((keypoints_2d, torch.ones(keypoints_2d.shape[0], keypoints_2d.shape[1], 1).type_as(keypoints_2d)), dim=-1).float().to(device) # [b,24,3]
